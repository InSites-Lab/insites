...
| אותנטיות | שלב 3 | Nara Grid כאובייקטים מובנים — היבט, תיאור, ביטוי ערך ודירוג — וסיכום |
...
**פתרון מיקום — מחייב לפני יצירת `DATA`:** קרא מחדש את שם הנכס, המיקום בשלב 0, הכותרות ותיאורי ההקשר. אם נמצא יישוב, אזור, כתובת או עוגן מקום חד־משמעי ומוכר אך לא נמסרה נקודה מדויקת, אין להשאיר את `coordinates` כ־`null` רק משום שחסרות קואורדינטות מדויקות. קבע נקודה מקורבת בעוגן המקום מתוך ידע גאוגרפי והגדר `coordinateSource:"inferred"`; זהו היסק מותר ולא המצאת מידע. אם העוגן עמום או אינו מזוהה בביטחון, אל תנחש: השתמש ב־`null` והוסף ל־`dataQuality.gaps` דיווח מסוים על מה לא זוהה או טעון אימות. מפת הנכס חייבת להיווצר בכל פעם שקיים עוגן מקום חד־משמעי.
...
טאבים קבועים, אוטומטיים ובסדר זה: **Overview · Map · Timeline · Contexts & Values · [Themes] · Integrity · Comparative · Significance**; אחריהם `tabs[]` הדינמיים ולבסוף **AI Query** פעיל. ב־GPT, ‏`host={}` ולכן AI Query פועל כהעתקה לצ'אט. Themes מוסתר כאשר קיימות פחות משתי תמות בסך הכול. ה־runtime מציג תוויות עבריות לפי שפת התוכן.
...
**Report / Debrief / Session Analysis כטאבים מסוג `prose`** — הפק בסדר זה לאחר Significance ובמזהים ובסמלים הבאים. את `label` כתוב בשפת הפלט:
...
אין לצרף מלל המשך לתשובת הארטיפקט. לאחר שהמשתמש מאשר ש־Preview נפתח, או שואל מה הלאה, הצע: ״האם תרצה: 1. **לייצא** את ההערכה כמסמך Word מעוצב? 2. להפעיל **Read-Assessment** ולנתח אותה מזוויות שונות? אפשר לבחור בשניהם, באחד מהם או באף אחד.״
...
- [ ] ה־runtime מ־`cdn.jsdelivr.net/npm/atar-runtime@0.3.6`; קריאת `mount(root, DATA, {})`; ומתקיים `DATA.type === "assessment"`.
...
- [ ] Themes מופיע רק כשיש לפחות שתי תמות; טאב Report מסוג `prose` קיים תמיד; Debrief ו־Session קיימים רק אם התרחשו.
