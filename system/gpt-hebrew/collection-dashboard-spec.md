...
# [CA-DB-C] דשבורד אוסף ‏(GPT · גרסת atar-runtime)

> **גרסת atar-runtime — המפרט הקנוני לדשבורד אוסף ב־GPT.** הרינדור מתבצע באמצעות חבילת **`atar-runtime`** המשותפת — JavaScript רגיל עם D3 ו־Leaflet מ־`cdn.jsdelivr.net/npm` — אותו runtime המשמש את Claude ואת Gemini. הבוט מחלץ נתונים בלבד; ה־runtime אחראי לכל הטאבים, למפה, ל־RTL ול־AI Query. גרסת `collection-dashboard-runtime` הישנה של alephplace נמצאת בארכיון.
...
טאבים קבועים ואוטומטיים: **Overview · Map · Values · [Themes]**; אחריהם `tabs[]` הדינמיים ולבסוף **AI Query** פעיל. ב־GPT, ‏`host={}` ולכן AI Query פועל כהעתקה לצ'אט. סמני המפה נצבעים לפי עומק `rich`/`medium`/`thin` וכוללים מסנן עומק. לחיצה על שם אתר בכל מקום ממקדת את הסמן שלו. ה־runtime מציג תוויות עבריות לפי שפת התוכן.
...
- קואורדינטות: חלץ, הסק או השתמש ב־`null`.

## 7. בדיקת תאימות

- [ ] הפלט הוא fenced block יחיד מסוג `html`, ובו מעטפת דקה בלבד: `<div id="root">` יחיד, סקריפט UMD, ‏`DATA` פנימי תקין ו־`mount`.
- [ ] ה־runtime מ־`cdn.jsdelivr.net/npm/atar-runtime@0.3.6`; קריאת `mount(root, DATA, {})`; ומתקיים `DATA.type === "collection"`.
