...
# kg-spec.md — גרף ידע CA-KG ‏(GPT · גרסת atar-runtime)

> **גרסת atar-runtime — מפרט ה־KG הקנוני ל־GPT.** הרינדור מתבצע באמצעות חבילת **`atar-runtime`** המשותפת — JavaScript רגיל ו־**D3**, הנטענים מ־`cdn.jsdelivr.net/npm` — אותו runtime המשמש את Claude ואת Gemini. גרסת `vis-network` הישנה של alephplace נמצאת בארכיון.

## מטרה

צור גרף ידע CA-KG כמעטפת HTML דקה הטוענת את `atar-runtime` וקוראת ל־`mount(container, DATA, host)`. ה־runtime אחראי לכל הרינדור: פריסת כוח D3, סרגל צד בעל שלושה טאבים, מקרא, חיפוש וסינון, זום וגרירה, הצגת מעמד אפיסטמי ו־RTL. צבעי הישויות נקבעים לפי `[CA-EC]`; ב־GPT הטאב AI Query פועל במצב מציין מקום של העתקה לצ'אט.
...
- טען את UMD של ה־runtime מכתובת jsDelivr המקובעת: `atar-runtime@0.3.6`.
...
⚠ החל את מדיניות השפה על כל שדות התוכן ב־KG. שמות השדות וערכי ה־enum נשארים באנגלית.
...
- הסימנים האפיסטמיים 💭/〰️ מופיעים בטאב Info וברשימת ״ישויות לבדיקה״ שב־Analytics, ולעולם לא על סמל הצומת.
...
<body>
  ...
  <script src="https://cdn.jsdelivr.net/npm/atar-runtime@0.3.6/dist/atar-runtime.umd.js"></script>
  <script>
    var DATA = {
      type: "kg",
      title: "{TITLE}",
...
רק `DATA` נכתב בתוך הקובץ. ה־runtime מזריק את הסגנונות והגופנים. ב־GPT, ‏`host` הוא `{}` ללא `window.claude.complete`, ולכן טאב AI Query מציג הצעות פתיחה ואפשרות העתקה לצ'אט.
...
גרף כוח D3 עם קשתות מעוקלות וראשי חץ; הזזה, זום וגרירה; בחירה בלחיצה והעמעמת צמתים שאינם שכנים; סרגל צד עם שלושה טאבים — **Info**, ‏**Analytics**, לרבות רשימת ״💭 ישויות לבדיקה״, ו־**AI Query** כמציין מקום; מקרא של סוגים קיימים בלבד; חיפוש וסינון לפי סוג; וזיהוי RTL אוטומטי לפי התוכן. מבני השדות המלאים נמצאים בחוזה הנתונים של `atar-runtime` עבור `type:'kg'`.
...
**בדיקת ישויות פרשניות — HITL:** באותה אינטראקציית המשך, כאשר הגרף כולל ישויות `interpretive`‏ (💭), הוסף הצעה של עד שני משפטים: ״בגרף יש N ישויות פרשניות 💭 — ראו ׳ישויות לבדיקה׳ בטאב Analytics. האם לאשר, לשנות שם, לדחות או להוסיף מקור ולקדם אחת מהן?״ בתגובה, שנה שם או הסר; קדם ל־`sourced` רק כאשר המשתמש הפנה לראיה; ולאחר מכן הצע ליצור את הגרף מחדש. דלג כאשר N = 0.

## בדיקת תאימות

- [ ] הפלט הוא fenced block יחיד מסוג `html`, ובו מעטפת דקה בלבד: `<div id="root">` יחיד, סקריפט UMD, ‏`DATA` פנימי תקין וקריאת `mount`.
- [ ] ה־runtime נטען מ־`cdn.jsdelivr.net/npm/atar-runtime@0.3.6`; נקראת `mount(root, DATA, {})`.
