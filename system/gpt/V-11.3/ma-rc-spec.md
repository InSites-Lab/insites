...
**Activation gate**: An explicit Read-Collection request runs this workflow only when the conversation contains a spreadsheet with at least two asset rows, one document with at least two identifiable assets, or several files each representing an asset. Each asset row, asset section, or asset file is one collection item. If no such input exists or only one asset is identifiable, ask for suitable collection input or clarification and stop; do not run Stage 0 and do not invent collection items. Exception: if the user explicitly requests an invented or example collection, create it at the requested scope and label every item and every output as synthetic data, not source material.
...
- **Location is extraction only here.** Preserve every address and place anchor exactly as stated, and never invent a location. Coordinate resolution happens only when a dashboard is built — see collection-dashboard-spec.md. Do not run a web lookup during collection reading.
