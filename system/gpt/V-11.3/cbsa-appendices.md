...
Inside `DATA`, use the exact English token from the first column. JSON keys, enum values and `[CA-EC]` tokens always stay English; `atar-runtime` renders the display label in the output language.
...
### Entity Types for KG
Keep the canonical English [CA-EC] `type` tokens inside the JSON. `atar-runtime` renders their visible labels as:
נכס מורשת, מקום, מבנה, אלמנט אדריכלי, דמות, אירוע, סיפור/נרטיב, ערך תרבותי, תופעה טבעית, יצירת אמנות/ממצא, מסורת/מנהג, קבוצה חברתית, תקופה היסטורית, דת/אמונה, זיכרון קולקטיבי

### Value Type Labels
היסטורי, אסתטי, חברתי, טכנולוגי, סמלי, נופי, מדעי, רוחני, סביבתי, אורבני, תיעודי, חינוכי, תפקודי
מסתורין וחידה
