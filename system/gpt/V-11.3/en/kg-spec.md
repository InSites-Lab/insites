...
If exact shell generation is blocked because the runtime/CDN/shell cannot be used, state the blocker and stop. A missing Preview control alone is not a shell-generation blocker; follow **GPT-5.6 HTML delivery** in `instructions.md`. Do not substitute another implementation.
...
**GPT delivery:** Follow **GPT-5.6 HTML delivery** in `instructions.md`; fallback filename: `{asset-name}-knowledge-graph.html`.
...
- [ ] `lang`/`dir` match the user's language; GPT delivery follows `instructions.md`; fallback filename is correct.
